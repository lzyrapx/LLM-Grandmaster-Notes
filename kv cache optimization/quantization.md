...
- NF4 在 KV Cache 中的实际局限性
    - NF4（NormalFloat 4）虽然在 QLoRA 中作为**权重**量化格式表现极佳，但在 **KV Cache 的实际部署中几乎无法使用** 。因为 KV Cache 的读取和计算是在自注意力机制（Attention Kernel）中高频发生的，NF4 没有标准的硬件乘法器支持，在 CUDA Kernel 中进行逐元素的非线性去量化开销过大，会严重拖慢推理速度。因此，实际工程中 4-bit KV Cache 几乎全部采用 **INT4** 或 **FP4**（NVFP4 / MXFP4）。
...
虽然学术界热衷于研究 INT4 甚至 2-bit (KIVI) 量化，但在目前的 **工业级推理框架（如 vLLM, TensorRT-LLM, LMDeploy）** 中，**FP8 (E4M3)** 是最主流、最成熟的落地方案：

- **硬件原生支持**：NVIDIA Hopper (H100/H200) 和 Blackwell 架构在硬件微架构层面原生都支持 FP8 的存储与计算。而 Blackwell 架构更是能支持 FP4 的存储与计算。

- **近乎无损**：使用 E4M3 格式进行 per-tensor 或 per-head 的 FP8 量化，在绝大多数百亿、千亿参数模型上能做到 **完全无损（Accuracy Loss ~ 0）**，且不需要任何校准数据集（Calibration）。FP4 量化需要 calibration 数据集。

- **极佳的吞吐收益**：FP8 KV 配合 PageAttention，能直接将最大 Batch Size 翻倍，带来非常稳健的吞吐线性能提升。

### 架构级替代方案：DeepSeek 提出的 MLA

对于 KV 压缩，除了纯粹的**后量化（PTQ）**，现在行业内也必须提及**架构设计上的压缩**。

- **对比**：DeepSeek-V2/V3 提出了 **MLA (Multi-head Latent Attention)** 架构。它在预训练阶段就通过低阶投影（Low-rank Joint Compression）将 KV 压缩成一个低维的潜在向量（Latent Vector）。

- **意义**：MLA 将 KV Cache 的显存占用直接砍掉了 **93% 左右**，在精度不减的前提下，其压缩效果远超任何后量化算法（如 INT4 仅压缩 75%）。

## 参考资料

- https://liner.com/review/kivi-tuningfree-asymmetric-2bit-quantization-for-kv-cache
- https://www.ijcai.org/proceedings/2025/0690.pdf
- https://briansu.co/articles/optimization/llm-serving
